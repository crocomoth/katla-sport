...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number){
    this.setStatusById(hiveSectionId, true);
  }

  onUnDelete(hiveSectionId: number){
    this.setStatusById(hiveSectionId, false);
  }

  private setStatusById(hiveId:number, status:boolean){
    var hiveSection = this.hiveSections.find(h => h.id == hiveId);
    this.hiveSectionService.setHiveSectionStatus(hiveSection.id, status).subscribe(h => hiveSection.isDeleted = status);
  }
}
